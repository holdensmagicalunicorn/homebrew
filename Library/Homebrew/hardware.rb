class Hardware
  ...
  def self.cpu_type
    @@cpu_type = 7 #||= `/sbin/sysctl -n hw.cputype`.to_i
  ...
  def self.intel_family
    @@intel_family = 0 #||= `/sbin/sysctl -n hw.cpufamily`.to_i
  ...
  def self.processor_count
    @@processor_count = 4 #||= `/sbin/sysctl -n hw.ncpu`.to_i
  end
  ...
  def self.is_64_bit?
    false #self.sysctl_bool("hw.cpu64bit_capable")
  end
